...
const ownerAccessMiddleware = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  try {
    ...
    if (data.accesstype == 2 || data.accesstype == 3) {
      return ApiError.handle(new AuthFailureError('Unauthorized'), res);
    }
    next();
  } catch (err) {
...
export default ownerAccessMiddleware;
